 ...
 */
const INVOICE_PHANTOM_FIELDS = new Set([
...
const QUOTE_PHANTOM_FIELDS = new Set([
    'currency',
    'clientEmail',
]);
class Frihet {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                // ===================================================================
                    ...
                    const clientName = this.getNodeParameter('clientName', i);
                    const itemsParam = this.getNodeParameter('items', i);
                    const additional = this.getNodeParameter('quoteAdditional', i, {});
                    const items_ = (itemsParam?.item ?? []).map((it) => ({
                        description: it.description,
                        quantity: it.quantity,
                        unitPrice: it.unitPrice,
                    ...
                    for (const [key, val] of Object.entries(additional)) {
                        if (val === '' || val === null || val === undefined)
                            continue;
                        if (QUOTE_PHANTOM_FIELDS.has(key))
                            continue;
                        if (key === 'clientAddress') {
                            try {
                                body.clientAddress = typeof val === 'string' ? JSON.parse(val) : val;
                ...
                // ===================================================================
                    ...
                    const additional = this.getNodeParameter('expenseAdditional', i, {});
                ...
                // ===================================================================
                    ...
                    const additional = this.getNodeParameter('clientAdditional', i, {});
                    const body = { name };
                    for (const [key, val] of Object.entries(additional)) {
                        if (val === '' || val === null || val === undefined)
                            continue;
                        if (key === 'address') {
                            try {
                                body.address = typeof val === 'string' ? JSON.parse(val) : val;
                            }
                            catch { /* ignore */ }
                        }
                        else {
                ...
                // ===================================================================
                    ...
                    const name = this.getNodeParameter('productName', i);
                    const unitPrice = this.getNodeParameter('unitPrice', i);
                    const additional = this.getNodeParameter('productAdditional', i, {});
                    const body = { name, unitPrice };
                    for (const [key, val] of Object.entries(additional)) {
                        if (val === '' || val === null || val === undefined)
                            continue;
                        body[key] = val;
                ...
                // ===================================================================
                    ...
                    const name = this.getNodeParameter('vendorName', i);
                    const additional = this.getNodeParameter('vendorAdditional', i, {});
                    const body = { name };
                    for (const [key, val] of Object.entries(additional)) {
                        if (val === '' || val === null || val === undefined)
                            continue;
                ...
                // ===================================================================
                    ...
                    const updateFields = this.getNodeParameter(updateParam, i, {});
                    // Strip empty values
                    const body = {};
